...
# def test_session_auth_401_if_no_prompting(session):
#     session.auth = MultiDomainBasicAuth(prompting=False)
#     resp = session.get("https://pypi.org/simple")
#     assert resp.status_code == 401


def test_session_auth_from_source_urls(session):
...
def test_session_auth_with_empty_password(session, monkeypatch):
...
# def test_session_auth_from_prompting(session):
...
# def test_session_auth_warn_agains_wrong_credentials(session, caplog):
