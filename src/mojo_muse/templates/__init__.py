...
class MojoProjectTemplate:
    ...
    def __enter__(self) -> MojoProjectTemplate:
        self._path = Path(tempfile.mkdtemp(suffix="-template", prefix="muse-"))
        self.prepare_template()
        return self
    ...
    def generate(
        ...
        self._generate_mojoproject(target_path / "mojoproject.toml", metadata)
    ...
    def _generate_mojoproject(self, path: Path, metadata: dict[str, Any]) -> None:
        try:
            with open(path, encoding="utf-8") as fp:
                content = tomlkit.load(fp)
        except FileNotFoundError:
            content = tomlkit.document()
        try:
            with open(self._path / "mojoproject.toml", encoding="utf-8") as fp:
                template_content = tomlkit.load(fp)
        except FileNotFoundError:
            template_content = tomlkit.document()
