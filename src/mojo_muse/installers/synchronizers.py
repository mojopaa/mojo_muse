...
def editables_candidate(project: Project) -> Candidate | None:
...
class BaseSynchronizer:
    ...
    def __init__(
        ...
        self.parallel = project.config["install.parallel"]
        self.fail_fast = fail_fast

        self.working_set = project.get_working_set()
        self.ui = project.ui
    ...
    def _should_update(self, dist: Distribution, can: Candidate) -> bool:
        ...
        if not can.req.is_named:
            dreq = MuseRequirement.from_dist(dist)  # TODO: check if no error
            if not isinstance(dreq, FileMuseRequirement):
                return True
    ...
    def compare_with_working_set(self) -> tuple[list[str], list[str], list[str]]:
        ...
        locked_repository = self.project.locked_repository
    ...
    def synchronize(self) -> None:
        ...
        for key in to_update:
            ...
            termui.logger.info(
                "Updating %s@%s -> %s...", key, dist_version, can.version
            )
    ...
    def _show_summary(self, packages: dict[str, list[str]]) -> None:
        ...
        if to_update:
            ...
            for prev, cur in to_update:
                lines.append(
                    f"  - [req]{cur.name}[/] [warning]{prev.version}[/] -> [warning]{cur.version}[/]"
                )
        if to_remove:
            lines.append("[bold]Packages to remove[/]:")
            for dist in to_remove:
                lines.append(
                    f"  - [req]{dist.metadata['Name']}[/] [warning]{dist.version}[/]"
                )
        if lines:
            self.ui.echo("\n".join(lines))
        else:
