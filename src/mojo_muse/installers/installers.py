...
from installer.destinations import Scheme, SchemeDictionaryDestination
...
_namespace_package_lines = _namespace_package_lines.union(
    line.replace("'", '"') for line in _namespace_package_lines
)
        ...
        if self.symlink_to:
            ...
            def _symlink_files(symlink_to: str) -> Iterator[tuple[Scheme, RecordEntry]]:
                for relpath in _create_symlinks_recursively(
                    symlink_to, self.scheme_dict[scheme]
                ):
...
def install_wheel(
    wheel: str, project: Project, direct_url: dict[str, Any] | None = None
) -> None:
    """Install a normal wheel file into the project."""  # no environment
    ...
    destination = InstallDestination(
        scheme_dict=project.get_paths(),  # TODO: PythonEnvironment.get_path, like sysconfig.get_path, and mojo's...
        interpreter=str(project.interpreter.executable),
        script_kind=_get_kind(project),
    )
    _install_wheel(
...
def _get_kind(project: Project) -> str:
    if os.name != "nt":
        return "posix"
    is_32bit = project.interpreter.is_32bit
