...
from .markers import Marker, PyMarker
from .project_file import PyProjectFile
...
class _SetupReader:
    ...
    def read_pyproject_toml(file: Path) -> Setup:
        try:
            metadata = PyProjectFile(file, ui=termui.UI()).metadata.unwrap()
        except ProjectError:
            return Setup()
        return Setup(
    ...
    def _find_install_requires(cls, call: ast.Call, body: Iterable[Any]) -> list[str]:
        ...
        if isinstance(value, ast.List):
            install_requires.extend(
                [el.s for el in value.elts if isinstance(el, ast.Str)]
            )
    ...
    def requires(self) -> list[str] | None:
        ...
        for extra, reqs in self._data.extras_require.items():
            ...
            for req in reqs:
                ...
                parsed.marker = PyMarker(
                    new_marker
                )  # TODO: check compatibility with PyMarker
