...
class ConfigItem:
    ...
    Args:
        ...
        default (Any): the default value, if given, will show in `muse config`
...
class Config(MutableMapping[str, str]):
    ...
    _config_map: ClassVar[dict[str, ConfigItem]] = {
        "cache_dir": ConfigItem(
            ...
            platformdirs.user_cache_dir("muse"),
            True,
        ),
        ...
        "global_project.path": ConfigItem(
            ...
            platformdirs.user_config_path("muse") / "global-project",
            True,
        ),
        ...
        "mojopi.url": ConfigItem(
            "The URL of MojoPI mirror, defaults to https://127.0.0.1:5000",
            DEFAULT_MOJOPI_INDEX,
            env_var="MUSE_MOJOPI_URL",
        ),
        "mojopi.verify_ssl": ConfigItem(
            "Verify SSL certificate when query MojoPI", True, coerce=ensure_boolean
        ),
        "mojopi.username": ConfigItem(
            "The username to access MojoPI", env_var="MUSE_MOJOPI_USERNAME"
        ),
        "mojopi.password": ConfigItem(
            "The password to access MojoPI", env_var="MUSE_MOJOPI_PASSWORD"
        ),
        "mojopi.ca_certs": ConfigItem(
            "Path to a CA certificate bundle used for verifying the identity of the MojoPI server",
            global_only=True,
        ),
        "mojopi.ignore_stored_index": ConfigItem(
            "Ignore the configured indexes",
            False,
            env_var="MUSE_IGNORE_STORED_INDEX",
            coerce=ensure_boolean,
        ),
        "mojopi.client_cert": ConfigItem(
            "Path to client certificate file, or combined cert/key file",
            global_only=True,
        ),
        "mojopi.client_key": ConfigItem(
            "Path to client cert keyfile, if not in pypi.client_cert", global_only=True
        ),
        "mojopi.json_api": ConfigItem(
            "Consult MojoPI's JSON API for package metadata",
            False,
            env_var="MUSE_MOJOPI_JSON_API",
            coerce=ensure_boolean,
        ),
        "venv.location": ConfigItem(
            "Parent directory for virtualenvs",
            os.path.join(platformdirs.user_data_dir("muse"), "venvs"),
            global_only=True,
        ),
        ...
        "venv.in_project": ConfigItem(
            ...
            default=True,
            env_var="MUSE_VENV_IN_PROJECT",
            coerce=ensure_boolean,
        ),
        "venv.prompt": ConfigItem(
            "Define a custom template to be displayed in the prompt when virtualenv is"
            "active. Variables `project_name` and `mojo_version` are available for"
            "formatting",
            default="{project_name}-{mojo_version}",
            env_var="MUSE_VENV_PROMPT",
        ),
        "venv.with_pip": ConfigItem(
            "Install pip when creating a new venv",
            default=False,
            env_var="MUSE_VENV_WITH_PIP",
            coerce=ensure_boolean,
        ),
    }
    _config_map.update(
    ...
    def get_defaults(cls) -> dict[str, Any]:
        ...
        if cls.site is None:
            cls.site = Config(platformdirs.site_config_path("muse") / "config.toml")
        ...
        for name, data in self._data.items():
            if name.startswith(f"{SOURCE}.") and name not in self._config_map and data:
                yield RepositoryConfig(
                    **data, name=name[len(SOURCE) + 1 :], config_prefix=SOURCE
                )

    def _save_config(self) -> None:
        """Save the changed to config file."""
        # TODO: replace with Tomlkit to save comments.
    ...
    def __getitem__(self, key: str) -> Any:
        ...
        if parts[0] in (REPOSITORY, SOURCE) and key not in self._config_map:
            if len(parts) < 2:
                raise MuseUsageError(f"Must specify a {parts[0]} name")
        ...
        if parts[0] in (REPOSITORY, SOURCE) and key not in self._config_map:
            if len(parts) < 3:
                raise MuseUsageError(
                    f"Set {parts[0]} config with [success]{parts[0]}.{{name}}.{{attr}}"
                )
            index_key = ".".join(parts[:2])
            username = self._data.get(index_key, {}).get("username")
            service = f'muse-{index_key.replace(".", "-")}'
            if (
                parts[2] == "password"
                and self.is_global
    ...
    def __delitem__(self, key: str) -> None:
        ...
        if parts[0] in (REPOSITORY, SOURCE) and key not in self._config_map:
            if len(parts) < 2:
                raise MuseUsageError(f"Should specify the name of {parts[0]}")
            if len(parts) >= 3:
        ...
        if env_var is not None and env_var in os.environ:
            ui.echo(
                "WARNING: the config is shadowed by env var '{}', set value won't take effect.".format(
                    env_var
                ),
                style="warning",
            )
    ...
    def get_repository_config(
        self, name_or_url: str, prefix: str
        ...
        if "://" in name_or_url:
            config = next(
                ...
                RepositoryConfig(
                    url=name_or_url, name="__unknown__", config_prefix=prefix
                ),
            )
        else:
        ...
        if name_or_url in DEFAULT_REPOSITORIES:
            if config is None:
                return RepositoryConfig(
                    url=DEFAULT_REPOSITORIES[name_or_url],
                    name=name_or_url,
