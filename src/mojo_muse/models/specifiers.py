"""Packaging's SpecifierSet and PySpecSet. PyVersion is alson in this module."""
...
from ..exceptions import InvalidPyVersion, deprecation_warning
from ..utils import resources_open_binary
...
class PyVersion:
    ...
    def complete(self, complete_with: VersionBit = 0, max_bits: int = 3) -> PyVersion:
        """
    ...
    def bump(self, idx: int = -1) -> PyVersion:
    ...
    def startswith(self, other: PyVersion) -> bool:
    ...
    def __repr__(self) -> str:
        return f"<PyVersion({self})>"

    def __eq__(self, other) -> bool:
        if not isinstance(other, PyVersion):
    ...
    def __lt__(self, other) -> bool:
        ...
        def comp_key(version: PyVersion) -> list[float]:
    ...
    @overload
    def __getitem__(self, idx: slice) -> PyVersion:
        ...

    def __getitem__(self, idx: int | slice) -> VersionBit | PyVersion:
...
PyVersion.MIN = PyVersion((-1, -1, -1))
PyVersion.MAX = PyVersion((99, 99, 99))
...
def _normalize_op_specifier(op: str, version_str: str) -> tuple[str, PyVersion]:
    version = PyVersion(version_str)
...
class PySpecSet(SpecifierSet):
    ...
    def __init__(self, specifiers: str = "", analyze: bool = True) -> None:
        ...
        self._lower_bound = PyVersion.MIN
        self._upper_bound = PyVersion.MAX
        self._excludes: list[PyVersion] = []
        if specifiers and analyze:
            self._analyze_specifiers()

    def _analyze_specifiers(self) -> None:
        lower_bound, upper_bound = PyVersion.MIN, PyVersion.MAX
        excludes: set[PyVersion] = set()
        for spec in self:
    ...
    def _merge_bounds_and_excludes(
        ...
        lower: PyVersion,
        upper: PyVersion,
        excludes: Iterable[PyVersion],
    ) -> tuple[PyVersion, PyVersion, list[PyVersion]]:
        ...
        if lower == PyVersion.MIN and upper == PyVersion.MAX:
    ...
    def _rearrange(
        self,
        lower_bound: PyVersion,
        upper_bound: PyVersion,
        excludes: Iterable[PyVersion],
    ) -> None:
    ...
    def _comp_key(self) -> tuple[PyVersion, PyVersion, tuple[PyVersion, ...]]:
    ...
    def is_impossible(self) -> bool:
        ...
        if self._lower_bound == PyVersion.MIN or self._upper_bound == PyVersion.MAX:
            return False
    ...
    def is_allow_all(self) -> bool:
        ...
        return (
            self._lower_bound == PyVersion.MIN
            and self._upper_bound == PyVersion.MAX
    ...
    def __str__(self) -> str:
        ...
        if upper[-1] == 0 and not upper.is_prerelease:
            upper = upper[:-1]
        lower_str = "" if lower == PyVersion.MIN else f">={lower}"
        upper_str = "" if upper == PyVersion.MAX else f"<{upper}"
    ...
    def _populate_version_range(
        self, lower: PyVersion, upper: PyVersion
    ) -> Iterable[PyVersion]:
    ...
    def is_subset(self, other: str | SpecifierSet) -> bool:
        ...
        if other._upper_bound >= self.MAX_MAJOR_VERSION:
            ...
            other._upper_bound = PyVersion.MAX
...
class ImpossiblePySpecSet(PySpecSet):
    ...
    def __init__(self, version_str: str = "", analyze: bool = True) -> None:
        ...
        self._lower_bound = PyVersion.MAX
        self._upper_bound = PyVersion.MIN
