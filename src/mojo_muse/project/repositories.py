...
from ..models.search import SearchResultParser
...
class BaseRepository:
    ...
    def get_dependencies(
    ...
    ) -> tuple[list[BaseMuseRequirement], PySpecSet, str]:
        ...
        candidate.requires_python = requires_python  # TODO
    ...
    def make_this_candidate(self, requirement: BaseMuseRequirement) -> Candidate:
        ...
        candidate.prepare(self.environment).metadata  # TODO
        return candidate
    ...
    def _get_dependency_from_local_package(self, candidate: Candidate) -> CandidateInfo:
        ...
        extra_dependencies = project.pyproject.settings.get(
            "dev-dependencies", {}
        ).copy()
    ...
    def search(self, query: str) -> SearchResult:
        ...
        with self.environment.get_finder() as finder:
            ...
            if resp.status_code == 404:
                self.environment.project.ui.echo(
...
class LockedRepository(BaseRepository):
        ...
        with cd(root):
            for package in lockfile.get("package", []):
                ...
                req_dict = {
                    ...
                    if k not in ("dependencies", "requires_python", "summary", "files")
                }
                req = MuseRequirement.from_req_dict(package_name, req_dict)
                if req.is_file_or_url and req.path and not req.url:  # type: ignore[attr-defined]  # TODO
    ...
    def find_candidates(
        ...
        for key in self._matching_keys(requirement):
            ...
            can.requires_python = info[1]  # TODO
